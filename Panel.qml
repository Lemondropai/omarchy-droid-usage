...
Panel {
  id: root
  moduleName: "io.github.lemondropai.droid-usage"
  ipcTarget: "io.github.lemondropai.droid-usage"
  manageIpc: false
  ...
  readonly property bool droidProviderAvailable: !!provider
    && String(provider.providerId || "") === "droid"

  property bool cursorActive: false
  property bool droidCoreSelected: String(usage.setting("droidUsagePool", "Standard")) === "Droid Core"
  readonly property string droidPoolPath: (Quickshell.env("HOME") || "") + "/.config/omarchy/droid-usage-pool"
  ...
  function selectDroidPool(core) {
    droidCoreSelected = core === true
    droidPoolFile.setText(droidCoreSelected ? "Droid Core\n" : "Standard\n")
  }

  function isDroidProvider(p) {
    return !!p && (String(p.providerId) === "droid"
      || p.droidCoreAvailable === true
      || String(p.providerName || "").toLowerCase() === "droid")
  }
  ...
  function launchAgent() {
    if (root.bar)
      root.bar.run("/usr/share/omarchy/bin/omarchy-launch-tui --app-id=org.omarchy.droid "
        + Quickshell.env("HOME") + "/.local/bin/droid")
    root.close()
  }
  ...
  function limitWindows(p) {
    if (!p) return []
    var out = []
    var list = isDroidProvider(p) && root.droidCoreSelected
      && p.coreLimits && p.coreLimits.length > 0
      ? p.coreLimits : (p.limits || [])
    for (var i = 0; i < list.length; i++) {
      var entry = list[i] || {}
      var percent = Number(entry.percent)
      if (percent >= 0) out.push(limitWindow(entry.label, percent, entry.resetsAt, entry.title))
    }
    var computers = isDroidProvider(p) ? p.managedComputers : null
    if (computers && Number(computers.percent) >= 0) {
      out.push({
        title: "Managed Computers",
        percent: Number(computers.percent),
        resetAt: String(computers.resetsAt || ""),
        detail: formatComputeUsage(computers.usedMs, computers.limitMs)
      })
    }
    return out
  }

  function formatComputeUsage(usedMs, limitMs) {
    var usedHours = Math.max(0, Number(usedMs) || 0) / 3600000
    var limitHours = Math.max(0, Number(limitMs) || 0) / 3600000
    var usedText = usedHours < 1 ? Math.round(usedHours * 60) + "m" : usedHours.toFixed(1) + "h"
    var limitText = Number.isInteger(limitHours) ? limitHours.toFixed(0) + "h" : limitHours.toFixed(1) + "h"
    return usedText + " used of " + limitText
  }
  ...
  function iconCandidatesForProvider(p, surfaceColor) {
    return [Qt.resolvedUrl("assets/factory.svg")]
  }

  FileView {
    id: droidPoolFile
    path: root.droidPoolPath
    watchChanges: true
    printErrors: false
    onFileChanged: reload()
    onLoaded: {
      var value = String(text() || "").trim()
      if (value === "Standard" || value === "Droid Core")
        root.droidCoreSelected = value === "Droid Core"
    }
  }
  ...
  BarIconButton {
    ...
    bar: root.bar
    text: ""
    tooltipText: "Droid Usage"
    iconComponent: Component {
      Image {
        source: Qt.resolvedUrl("assets/factory.svg")
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          Row {
            id: droidPoolSwitch
            visible: root.droidProviderAvailable
            width: parent.width
            spacing: Style.spacing.md

            Button {
              id: standardPoolButton
              width: (parent.width - droidPoolSwitch.spacing) / 2
              text: "Standard"
              selected: !root.droidCoreSelected
              bordered: true
              foreground: root.foreground
              fontFamily: root.fontFamily
              fontSize: Style.font.bodySmall
              verticalPadding: Style.spacing.controlPaddingY
              onClicked: root.selectDroidPool(false)
            }
            ...
            Button {
              ...
              width: (parent.width - droidPoolSwitch.spacing) / 2
              text: "Droid Core"
              enabled: !!root.provider && root.provider.droidCoreAvailable === true
              selected: root.droidCoreSelected
              bordered: true
              foreground: root.foreground
              fontFamily: root.fontFamily
              fontSize: Style.font.bodySmall
              verticalPadding: Style.spacing.controlPaddingY
              onClicked: root.selectDroidPool(true)
            }
          }
  ...
  component LimitRow: Column {
    ...
    Text {
      ...
      text: {
        ...
        var parts = []
        if (limitRow.window && String(limitRow.window.detail || "") !== "")
          parts.push(String(limitRow.window.detail))
        if (remainingMs > 0) parts.push("Resets in " + root.formatDuration(remainingMs))
        return parts.join(" · ")
      }
      color: root.dim
      font.family: root.fontFamily
