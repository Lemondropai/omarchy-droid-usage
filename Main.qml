...
Item {
  ...
  readonly property string droidCollectorPath: Qt.resolvedUrl("droid-usage.mjs").toString().replace(/^file:\/\//, "")
  ...
  Timer {
    ...
    interval: 30000
    repeat: false
    onTriggered: root.runDroidUpdate("limits")
  }
  ...
  // Match the Factory Usage page, which refreshes account limits every 20
  // seconds. Droid's collector only reads two small account endpoints.
  Timer {
    interval: 20000
    running: root.droidEnabled()
    repeat: true
    triggeredOnStart: true
    onTriggered: root.runDroidUpdate("normal")
  }
  ...
  property string pendingDroidUpdateKind: ""

  Process {
    id: droidUpdateProcess
    running: false
    command: ["node", root.droidCollectorPath]
    onExited: {
      root.rescanAgents()
      if (root.pendingDroidUpdateKind !== "") {
        var kind = root.pendingDroidUpdateKind
        root.pendingDroidUpdateKind = ""
        root.runDroidUpdate(kind)
      }
    }

    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("agents/droid", text.trim())
    }

    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.applyDroidRecord(text)
    }
  }
  ...
  function droidEnabled() {
    return providerEnabled("droid")
  }

  function runDroidUpdate(kind) {
    if (!droidEnabled()) return
    if (droidUpdateProcess.running) {
      if (kind === "force" || root.pendingDroidUpdateKind === "")
        root.pendingDroidUpdateKind = kind
      return
    }
    droidUpdateProcess.running = true
  }

  // Atomic file replacement is safe for the snapshot but not every file
  // watcher reports the rename promptly. Apply the collector's stdout
  // immediately so an open panel always receives the newest values.
  function applyDroidRecord(output) {
    try {
      var parsed = JSON.parse(String(output || ""))
      if (!parsed || String(parsed.id || "") !== "droid") return
      for (var i = 0; i < agents.length; i++) {
        if (agents[i] && String(agents[i].agentId) === "droid") {
          agents[i].record = parsed
          return
        }
      }
    } catch (e) {
      console.warn("agents/droid", "Ignoring bad collector output", e)
    }
  }

  function refresh() { refreshAll(true) }
  function refreshAll(force) {
    var kind = force === true ? "force" : "normal"
    runDroidUpdate(kind)
  }
  ...
  function providerEnabled(id) {
    if (String(id) !== "droid") return false
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      coreLimits: Array.isArray(record.coreLimits) ? record.coreLimits : [],
      droidCoreAvailable: record.droidCoreAvailable === true,
      managedComputers: record.managedComputers && typeof record.managedComputers === "object"
        ? record.managedComputers : null,
